...
void GhostManager::reset(unsigned char i_level, const std::array<Position, 4>& i_ghost_positions)
{
	...
	for (Ghost& ghost : ghosts)
	{
		// 문을 통한 집의 위치를 얻기위해 파란 유령을 사용하고 나가는 길을 찾기 위해 빨간 유령을 사용합니다.
...
{
	if (0 == i_pacman.get_Invisible_timer()) // 팩맨이 일반모드일 때만 웨이브 타이머를 활성화합니다.
		//7초 동안 산란 후 20초 동안 추적합니다.
		//7초 동안 산란 후 20초 동안 추적합니다.
		//5초 동안 산란 후 20초 동안 추적합니다.
		//5초 동안 흩어졌다가 영구적으로 추적 모드로 전환합니다.
	{
		if (0 == wave_timer)
		{
			if (7 > current_wave)
