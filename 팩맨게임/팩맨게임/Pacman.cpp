...
unsigned short Pacman::get_Invisible_timer()
{
	return Invisible_timer;
}

void Pacman::draw(bool i_victory, sf::RenderWindow& i_window, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map)
{
	...
	if (1 == dead || 1 == i_victory)
	{
		...
		{
			animation_timer++;
			texture.loadFromFile("PacmanDeath16.png");
	...
	{
		texture.loadFromFile("Pacman16.png");
		sprite.setTexture(texture);
...
void Pacman::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map)
{
	...
	{
		Invisible_timer = static_cast<unsigned short>(Invisible_DURATION / pow(2, i_level));
	}
	else
	{
		Invisible_timer = std::max(0, Invisible_timer - 1);
	}
}
